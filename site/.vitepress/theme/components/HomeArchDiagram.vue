...
const ctaLink = `${base}tutorial/`

const layers = [
  {
    icon: '👤',
    label: '应用任务层',
    sublabel: 'Application Tasks',
    items: ['多任务传感器仪表盘', 'main_blinky', 'main_full']
  },
  {
    icon: '🧠',
    label: 'FreeRTOS 内核',
    sublabel: 'Kernel',
    items: ['调度器', '队列 / 信号量', '定时器 / 事件组', '任务通知']
  },
  {
    icon: '🔌',
    label: '移植层',
    sublabel: 'POSIX / MSVC Port',
    items: ['GCC Posix port', 'WIN32-MSVC port']
  },
  {
    icon: '🛠️',
    label: 'Mock HAL + Bridge',
    sublabel: 'Simulated Peripherals',
    flow: '模拟外设 pthread → IPC → bridge task → FromISR'
  },
  {
    icon: '💻',
    label: '主机 OS + PC',
    sublabel: 'Host',
    items: ['Linux / macOS / Windows', 'x86 / ARM64', 'WSL2']
  }
]
</script>
  ...
  <div class="home-arch">
    <div class="home-arch-inner">
      <div class="home-arch-header">
        <h2 class="home-arch-title">主机模拟架构概览</h2>
        <p class="home-arch-desc">FreeRTOS 内核、移植层与 Mock HAL 如何协作，是理解本教程双轨模拟的第一步</p>
      </div>
      ...
      <div class="arch-diagram">
        ...
        <div class="arch-boot-axis">
          <span class="arch-boot-label">信号注入</span>
